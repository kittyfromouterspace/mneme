...
- `mix mneme.gen.migration --dimensions N` — generate the database migration. You must run `mix ecto.migrate` afterward to apply it.

## Architecture

- Single-package Elixir library (not a monorepo). All code lives in `lib/mneme/`.
...
- `Mneme.EmbeddingProvider` — add a new embedding backend (e.g. local Ollama).
- `Mneme.ExtractionProvider` — customize entity/relation extraction logic.
- `Mneme.GraphStore` — swap the graph backend (default: PostgresGraph using recursive CTEs).
