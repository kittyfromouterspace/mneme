defmodule Mneme.MipmapTest do
  ...
  describe "Mneme.Mipmap.determine_level/1" do
    test "returns abstract for short queries" do
      assert Mneme.Mipmap.determine_level("auth") == :abstract
    end

    test "returns summary for medium queries" do
      query = String.duplicate("a", 100)
      assert Mneme.Mipmap.determine_level(query) == :summary
    end

    test "returns full for long queries" do
      query = String.duplicate("a", 250)
      assert Mneme.Mipmap.determine_level(query) == :full
    end
  end

  describe "Mneme.Mipmap.levels/0" do
    test "returns all levels" do
      assert Mneme.Mipmap.levels() == [:anchor, :abstract, :summary, :full]
    end
  end
end
