defmodule Recollect.Search.Completion do
  @moduledoc """
  ...
  def complete(question, opts \\ []) do
    ...
    Recollect.Telemetry.span([:recollect, :completion], metadata, fn ->
      with {:ok, context_pack} <- Search.search(question, search_opts) do
        ...
        case llm_fn.(messages) do
          ...
          {:error, reason} ->
            Logger.error("Recollect.Completion: LLM call failed: #{inspect(reason)}")
            {:error, reason}
        end
      end
    end)
