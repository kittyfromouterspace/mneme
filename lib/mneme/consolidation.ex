defmodule Mneme.Consolidation do
  ...
  def run(opts \\ []) do
    ...
    Telemetry.event([:mneme, :consolidation, :stop], result)
  ...
  defp fetch_owner_id(scope_id, repo) do
    case repo.query("SELECT owner_id FROM mneme_entries WHERE scope_id = $1 LIMIT 1", [
           uuid_to_bin(scope_id)
         ]) do
  ...
  defp decay_pass(entries, threshold) do
    ...
    {survivors, removed} =
      Enum.split_with(entries, fn entry ->
        entry = struct(Mneme.Schema.Entry, entry)
