defmodule Mneme.MigrationGenerator do
  @moduledoc """
  Generates database-specific migration content for Mneme tables.

  This module provides adapter-aware migration generation, allowing
  Mneme to work with different database backends (PostgreSQL, libSQL).

  ## Usage

      # In a migration file
      defmodule MyApp.Repo.Migrations.CreateMnemeTables do
        use Ecto.Migration

        def up do
          Mneme.MigrationGenerator.generate_up(Mneme.DatabaseAdapter.LibSQL, dimensions: 768)
        end

        def down do
          Mneme.MigrationGenerator.generate_down()
        end
      end
  """
  ...
  @doc """
  Generates the `up` migration content for creating all Mneme tables.

  ## Options

  - `:dimensions` — Vector embedding dimensions (default: 1536)
  - `:table_prefix` — Table prefix (default: "mneme_")
  """
  def generate_up(adapter \\ Config.adapter(), opts \\ []) do
    dimensions = Keyword.get(opts, :dimensions, 1536)
    prefix = Keyword.get(opts, :table_prefix, "mneme_")
  ...
  @doc """
  Generates the `down` migration content for dropping all Mneme tables.
  """
