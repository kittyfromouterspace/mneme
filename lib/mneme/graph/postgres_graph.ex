defmodule Mneme.Graph.PostgresGraph do
  @moduledoc """
  Graph store implementation using PostgreSQL recursive CTEs.
  Queries the mneme_entities and mneme_relations tables directly.
  """
  ...
  def get_neighbors(owner_id, entity_id, hops) do
    ...
    WITH RECURSIVE graph_walk AS (
      ...
      SELECT
        ...
        gw.depth + 1
      FROM graph_walk gw
      JOIN mneme_relations r ON (r.from_entity_id = gw.entity_id OR r.to_entity_id = gw.entity_id)
      WHERE gw.depth < $3
        AND r.owner_id = $2::uuid
    )
    SELECT DISTINCT
    ...
    FROM graph_walk gw
    JOIN mneme_entities e ON e.id = gw.entity_id
    WHERE e.id != $1::uuid
    LIMIT 50
    """
