defmodule Mneme.Search.Graph do
  @moduledoc """
  ...
  def neighborhood(entity_id, opts \\ []) do
    ...
    Mneme.GraphStore.impl().get_neighbors(owner_id, entity_id, hops)
  end
  ...
  def relations(entity_id, opts \\ []) do
    owner_id = Keyword.fetch!(opts, :owner_id)
    Mneme.GraphStore.impl().get_relations(owner_id, entity_id)
  end
    ...
    if entry_ids == [] do
      ...
      WITH RECURSIVE edge_walk AS (
        ...
        SELECT
          ...
          ew.depth + 1
        FROM edge_walk ew
        JOIN mneme_edges e ON (e.source_entry_id = ew.entry_id OR e.target_entry_id = ew.entry_id)
        WHERE ew.depth < $2
      )
      SELECT DISTINCT me.id, me.content, me.summary, me.entry_type, me.confidence, me.inserted_at
      FROM edge_walk ew
      JOIN mneme_entries me ON me.id = ew.entry_id
      WHERE me.id != ALL($1)
      ...
      case repo.query(sql, [bin_ids, hops, limit]) do
        ...
        {:error, reason} ->
          Logger.error("Mneme edge traversal failed: #{inspect(reason)}")
          {:error, reason}
      end
    end
  end
